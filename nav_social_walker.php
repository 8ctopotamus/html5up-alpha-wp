...
class WO_Nav_Social_Walker extends Walker_Nav_Menu {
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        if (strpos($item->url, 'facebook') !== false) {
            $item_output .= '<li><a'. $attributes .' class="icon fa-facebook-f"></a></li>';
            $item_output .= $args->after;
        } elseif (strpos($item->url, 'twitter') !== false)  {
            $item_output .= '<li><a'. $attributes .' class="icon fa-twitter"></a></li>';
            $item_output .= $args->after;
        } elseif (strpos($item->url, 'instagram') !== false)  {
            $item_output .= '<li><a'. $attributes .' class="icon fa-instagram"></a></li>';
            $item_output .= $args->after;
        } elseif (strpos($item->url, 'youtube') !== false)  {
            $item_output .= '<li><a'. $attributes .' class="icon fa-youtube-play"></a></li>';
            $item_output .= $args->after;
        }  elseif (strpos($item->url, 'snapchat') !== false)  {
            $item_output .= '<li><a'. $attributes .' class="icon fa-snapchat"><i></a></li>';
            $item_output .= $args->after;
        } elseif (strpos($item->url, 'vimeo') !== false)  {
            $item_output .= '<li><a'. $attributes .' class="icon fa-vimeo-v"></a></li>';
            $item_output .= $args->after;
        } 
